...
struct configuration {
    ...
    /// Admission control configuration applied to the per-shard
    /// cloud_storage_clients::client_pool. Populated once at startup
    /// from cluster config; passed by value down to the pool and
    /// admission_control so neither re-reads cluster state.
    cloud_io::admission_control_config admission_control;
